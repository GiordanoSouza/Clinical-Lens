...
> **Important:** Convex vector search API may require using `ctx.vectorSearch()` inside an **action** rather than a query. If so, create this alternative:

**File: `convex/actions.ts`** (alternative vector search using action)
...
- [ ] `data/` directory with all 6 downloaded CSV.gz files (or in `.gitignore` with download script)
- [ ] `scripts/download-data.ts` — downloads all datasets
- [ ] `scripts/parse-csv.ts` — CSV.gz parser utility
- [ ] `scripts/seed-convex.ts` — seeds all 6 tables into Convex
- [ ] `scripts/generate-embeddings.ts` — generates + stores embeddings
- [ ] `scripts/verify-seed.ts` — verification script passes
- [ ] `convex/mutations.ts` — all seed mutations + batch variants
- [ ] `convex/queries.ts` — all 7 query functions
- [ ] `convex/actions.ts` — vector search action (if needed)
- [ ] Convex dashboard shows all tables populated with data
- [ ] Vector index `by_embedding` has embeddings for ~2000 cases
- [ ] `data/` added to `.gitignore` (don't commit raw CSVs)
...
| Embedding API errors | Check OpenAI API key, ensure text is under 8192 tokens |
